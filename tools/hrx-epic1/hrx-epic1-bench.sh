...
ROCM_PATH="${GGML_HRX_ROCM_PATH:-${WORKSPACE_ROOT}/build/therock/dist/rocm}"
HRX_RUNTIME_SRC="${HRX_RUNTIME_SRC:-${WORKSPACE_ROOT}/sources/pyre-runtime}"
HRX_RUNTIME_BUILD="${HRX_RUNTIME_BUILD:-${BUILD_ROOT}/hrx-runtime}"
HRX_RUNTIME_INSTALL="${HRX_RUNTIME_INSTALL:-${BUILD_ROOT}/hrx-runtime-install}"

CPU_BUILD="${BUILD_ROOT}/llama-cpu"
VULKAN_BUILD="${BUILD_ROOT}/llama-vulkan"
HIP_BUILD="${BUILD_ROOT}/llama-hip"
HRX_BUILD="${BUILD_ROOT}/llama-hrx"
...
OUTPUT_DIR="${LLAMA_BENCH_OUTPUT_DIR:-${BUILD_ROOT}/hrx-epic1-results}"

DEFAULT_VARIANTS=(cpu vulkan hip hrx-fallback hrx-rmsnorm)

usage() {
    cat <<'EOF'
Usage:
  hrx-epic1-bench.sh build [variant...]
  hrx-epic1-bench.sh smoke [variant...]
  hrx-epic1-bench.sh bench [variant...]

Variants:
  cpu vulkan hip hrx-fallback hrx-rmsnorm hrx
...
Environment overrides:
  ...
  GGML_HRX_ROCM_PATH=/path/to/rocm
  GGML_HRX_CLANGXX=/path/to/clang++
EOF
}
...
build_hrx_runtime() {
    rm -f "${HRX_RUNTIME_BUILD}/CMakeCache.txt"
    ...
    case "${variant}" in
        ...
        hrx|hrx-fallback|hrx-rmsnorm)
            ...
            cmake -S "${LLAMA_SRC}" -B "${HRX_BUILD}" \
                ...
                $(cmake_common_args)
            cmake --build "${HRX_BUILD}" --target llama-bench test-backend-hrx -j"$(nproc)"
            ;;
        *)
...
default_model_path() {
    ...
    if [[ -f /srv/vm-shared/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf ]]; then
        printf '%s\n' /srv/vm-shared/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf
        return
    fi
...
variant_binary() {
    case "$1" in
        ...
        hrx|hrx-fallback|hrx-rmsnorm) printf '%s\n' "${HRX_BUILD}/bin/llama-bench" ;;
        *) return 1 ;;
    esac
}
...
run_variant() {
    ...
    case "${variant}" in
        ...
        hip)
            ...
            ;;
        hrx-fallback)
            env_prefix+=(GGML_HRX_DISABLE_RMS_NORM=1)
    ...
    env \
        LD_LIBRARY_PATH="${ROCM_PATH}/lib:${HRX_RUNTIME_INSTALL}/lib64:${LD_LIBRARY_PATH:-}" \
        "${env_prefix[@]}" \
        "${args[@]}" >"${output_path}"
