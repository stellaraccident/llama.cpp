#!/usr/bin/env python3
"""Summarize HRX provider traces and Vulkan perf logger fusion timings."""
...
HRX_FALLBACK_RE = re.compile(r"ggml-hrx\[[^]]+\]: fallback ([A-Z0-9_]+)(.*)")
HRX_SUMMARY_RE = re.compile(r"provider summary (.*)$")
...
# Some HRX providers intentionally cover the same graph region under a different
...
def summarize_hrx(log_paths: list[Path], top: int) -> tuple[Counter[str], list[str]]:
    ...
    for log_path in log_paths:
        with log_path.open("r", encoding="utf-8", errors="replace") as f:
            for line in f:
                if match := HRX_CLAIM_RE.search(line):
                    ...
                    if shape:
                        shapes[(key, shape)] += 1
                    continue
                if match := HRX_FALLBACK_RE.search(line):
                    op, tail = match.groups()
                    reason = tail.strip() or "unknown"
                    fallbacks[f"{op} {reason}"] += 1
                    continue
                if match := HRX_SUMMARY_RE.search(line):
...
def compare_hrx_vulkan(hrx_ops: Counter[str], vulkan_fusions: Counter[str]) -> None:
    print("\nVulkan fusion labels without a direct HRX equivalent in this trace")
    missing = []
    hrx_op_names = set(hrx_ops)
    for fusion, count in vulkan_fusions.items():
        if fusion == "<unfused>":
            continue
        equivalents = HRX_EQUIVALENTS.get(fusion, {fusion})
        if not equivalents.intersection(hrx_op_names):
    ...
    for fusion, count, equivalents in sorted(missing, key=lambda item: (-item[1], item[0])):
        print(f"{count:7d}  {fusion}  expected-hrx={equivalents}")
    if not missing:
        print("      0  <none>")
...
def command_env(base: dict[str, str], binary_dir: Path, extra: dict[str, str]) -> dict[str, str]:
    env = dict(base)
    root = workspace_root()
    rocm = Path(env.get("GGML_HRX_ROCM_PATH") or env.get("ROCM_PATH") or root / "rocm")
    hrx_install = Path(env.get("HRX_RUNTIME_INSTALL") or root / "build/hrx-runtime-rocm713-install")
    ld_parts = [
        str(binary_dir),
        str(rocm / "lib"),
        str(rocm / "lib/rocm_sysdeps/lib"),
        str(hrx_install / "lib64"),
...
def run_trace(args: argparse.Namespace) -> int:
    ...
    backends = ["hrx", "vulkan"] if args.backend == "both" else [args.backend]
    hrx_ops = Counter()
    vulkan_fusions = Counter()
    rc = 0
    for backend in backends:
        if backend == "hrx":
            binary = root / "build/llama-hrx-rocm713/bin/llama-bench"
            dev_args = ["-ngl", str(args.gpu_layers), "-dev", "HRX0"]
        ...
        if args.summarize:
            if backend == "hrx":
                hrx_ops, _fallbacks = summarize_hrx([stderr_path], args.top)
            else:
                vulkan_fusions = summarize_vulkan([stderr_path], args.top)
            print()
    if args.summarize and hrx_ops and vulkan_fusions:
        compare_hrx_vulkan(hrx_ops, vulkan_fusions)
    return rc
...
def main() -> int:
    ...
    summarize = subparsers.add_parser("summarize", help="summarize existing HRX/Vulkan logs")
    summarize.add_argument("--hrx-log", action="append", type=Path, default=[])
    ...
    run.add_argument("--backend", choices=("hrx", "vulkan", "both"), default="both")
    ...
    run.add_argument("--output-dir", default=str(workspace_root() / "build/hrx-epic2-results/trace-diff"))
    ...
    if argv and argv[0] not in {"summarize", "run-qwen", "-h", "--help"} and not argv[0].startswith("-"):
        ...
        #   hrx-trace-summary.py trace.log
        # Treat positional paths as HRX provider logs.
        argv = ["summarize", "--hrx-log", argv[0], *argv[1:]]
    ...
    vulkan_fusions = Counter()
    if args.hrx_log:
        hrx_ops, _fallbacks = summarize_hrx(args.hrx_log, args.top)
    if args.hrx_log and args.vulkan_log:
        print()
    if args.vulkan_log:
        vulkan_fusions = summarize_vulkan(args.vulkan_log, args.top)
    if args.hrx_log and args.vulkan_log:
        compare_hrx_vulkan(hrx_ops, vulkan_fusions)
    if not args.hrx_log and not args.vulkan_log:
        parser.error("summarize requires --hrx-log and/or --vulkan-log")
    return 0
