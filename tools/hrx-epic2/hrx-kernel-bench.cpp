...
static void usage(const char * argv0) {
    std::fprintf(stderr,
        ...
        "\n"
        "Benchmarks ggml-hrx providers without a GGUF model.\n",
        argv0);
}
...
static bool uses_q8_1_rhs(const bench_options & options) {
    if (!env_enabled("GGML_HRX_ENABLE_Q8_1_MMVQ") || env_enabled("GGML_HRX_DISABLE_Q8_1_MMVQ")) {
        return false;
    }
    const bool force_all = [] {
        const char * policy = std::getenv("GGML_HRX_Q8_1_MMVQ_POLICY");
...
int main(int argc, char ** argv) {
    ...
    if (!dev) {
        std::fprintf(stderr, "HRX0 not available; skipping hrx-kernel-bench\n");
        return 0;
    }

    ggml_backend_ptr backend(ggml_backend_dev_init(dev, nullptr));
    if (!backend) {
        std::fprintf(stderr, "failed to initialize HRX backend\n");
        return 1;
    }
    ...
    if (!ggml_backend_dev_supports_op(dev, dst)) {
        std::fprintf(stderr, "HRX provider is unavailable for op %s on this build/device\n", options.op.c_str());
        return 0;
    }
    ...
    if (!buffer) {
        std::fprintf(stderr, "failed to allocate HRX graph buffer\n");
        return 1;
    }
