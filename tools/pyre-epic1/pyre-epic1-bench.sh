...
OUTPUT_DIR="${LLAMA_BENCH_OUTPUT_DIR:-${BUILD_ROOT}/pyre-epic1-results}"

DEFAULT_VARIANTS=(cpu vulkan hip pyre-fallback pyre-rmsnorm)

usage() {
    cat <<'EOF'
Usage:
  pyre-epic1-bench.sh build [variant...]
  pyre-epic1-bench.sh smoke [variant...]
  pyre-epic1-bench.sh bench [variant...]

Variants:
  cpu vulkan hip pyre-fallback pyre-rmsnorm pyre
...
Environment overrides:
  ...
  GGML_PYRE_ROCM_PATH=/path/to/rocm
  GGML_PYRE_CLANGXX=/path/to/clang++
EOF
}
...
build_pyre_runtime() {
    rm -f "${PYRE_RUNTIME_BUILD}/CMakeCache.txt"
...
build_llama_variant() {
    ...
    case "${variant}" in
        ...
        hip)
            ...
            ;;
        pyre|pyre-fallback|pyre-rmsnorm)
            build_pyre_runtime
            cmake -S "${LLAMA_SRC}" -B "${PYRE_BUILD}" \
                ...
                $(cmake_common_args)
            cmake --build "${PYRE_BUILD}" --target llama-bench test-backend-pyre -j"$(nproc)"
            ;;
        *)
...
variant_binary() {
    case "$1" in
        ...
        pyre|pyre-fallback|pyre-rmsnorm) printf '%s\n' "${PYRE_BUILD}/bin/llama-bench" ;;
        *) return 1 ;;
    esac
}
...
run_variant() {
    ...
    case "${variant}" in
        ...
        hip)
            ...
            ;;
        pyre-fallback)
            env_prefix+=(GGML_PYRE_DISABLE_RMS_NORM=1)
    ...
    env \
        LD_LIBRARY_PATH="${ROCM_PATH}/lib:${PYRE_RUNTIME_INSTALL}/lib64:${LD_LIBRARY_PATH:-}" \
        "${env_prefix[@]}" \
        "${args[@]}" >"${output_path}"
