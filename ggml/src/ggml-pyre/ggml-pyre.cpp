...
struct ggml_backend_pyre_device_context {
    ...
    ggml_backend_pyre_op_provider mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider;
...
static bool ggml_backend_pyre_load_mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider(
        ggml_backend_pyre_device_context * device_context) {
    return ggml_backend_pyre_load_catalog_provider(
        device_context,
        ggml_backend_pyre_find_catalog_entry("pyre_mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_f32"),
        &device_context->mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider);
}
...
static ggml_status ggml_backend_pyre_dispatch_flash_attn_ext_f32_f16_decode(
    ...
    const bool use_prefill_tile =
        ggml_backend_pyre_supports_flash_attn_ext_f32_f16_prefill_tile(context->device_context, dst);
    const ggml_backend_pyre_op_provider * selected_provider = use_prefill_tile ?
    ...
    pyre_dispatch_config_t config = {
        /* .workgroup_count = */ {
            static_cast<uint32_t>(use_prefill_tile ? ((constants.N + 7) / 8) : constants.H),
            static_cast<uint32_t>(use_prefill_tile ? constants.H : constants.N),
...
static bool ggml_backend_pyre_supports_mul_mat_vec_q6_k_q8_1_x4_mmql128_prompt(
    ...
    return device_context->policy.enable_q6_k_q8_1_x4_mmql128_prompt &&
           ...
           ggml_backend_pyre_supports_mul_mat_vec_k_quant(
               device_context, op, GGML_TYPE_Q6_K,
               device_context->mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider, 256);
}
...
static bool ggml_backend_pyre_supports_mul_mat_vec_q8_0_add(
    ...
    if ((!has_scalar_provider && !has_cols8_provider) ||
...
static ggml_status ggml_backend_pyre_dispatch_mul_mat_vec_k_q8_1(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_Q6_K:
            if (use_q6_x4_mmql128) {
                provider = &context->device_context->mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider;
    ...
    const uint32_t provider_cols_per_workgroup =
        ...
        (provider == &context->device_context->mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider) ? 128 :
        ...
        pyre_dispatch_config_t config = {
            /* .workgroup_count = */ {
                ...
                static_cast<uint32_t>(
                    (use_f32_cols16 ? constants.cols / 16 :
                        (use_f32_cols8 ? constants.cols / 8 :
                            (use_f16_cols16 ? constants.cols / 16 :
...
static ggml_status ggml_backend_pyre_dispatch_mul_mat_vec_bf16_swiglu(
    ...
    pyre_dispatch_config_t config = {
        /* .workgroup_count = */ {
            static_cast<uint32_t>(constants.rows),
            static_cast<uint32_t>(
                &provider == &context->device_context->mul_mat_vec_bf16_swiglu_cols16_provider ?
                    (constants.cols + 15) / 16 :
                (&provider == &context->device_context->mul_mat_vec_bf16_swiglu_cols8_provider ?
                    (constants.cols + 7) / 8 :
    ...
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        ...
        switch (node->op) {
            ...
            case GGML_OP_FLASH_ATTN_EXT:
                ...
                if (ggml_backend_pyre_supports_flash_attn_ext_f32_f16_prefill_tile(context->device_context, node)) {
                    ggml_backend_pyre_trace_provider(
                        context->device_context,
...
static std::unique_ptr<ggml_backend_pyre_reg_context> ggml_backend_pyre_create_reg_context() {
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        if (device_context->policy.kernel_provider == ggml_backend_pyre_kernel_provider_mode::pure_hip &&
            ...
            (void) ggml_backend_pyre_load_mul_mat_vec_q6_k_q8_1_x4_mmql128x128_wg256_provider(device_context.get());
