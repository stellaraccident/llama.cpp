...
struct pyre_mul_mat_vec_bf16_set_rows_constants {
    long long k;
    long long rows;
...
static __device__ __forceinline__ long long pyre_load_i64(const long long * base, long long byte_offset) {
...
extern "C" __global__ void pyre_mul_mat_vec_bf16_set_rows_f16(
        const uint16_t * src0, const float * src1, const long long * idxs, __half * dst,
        pyre_mul_mat_vec_bf16_set_rows_constants c) {
    ...
    if (tid == 0) {
        const long long dst_row = pyre_load_i64(idxs, row * c.idx_nb0);
