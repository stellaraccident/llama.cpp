...
extern "C" __global__ __launch_bounds__(256) void pyre_flash_attn_ext_f32_f16_prefill_wmma16(
        const float * q,
        const __half * k,
        const __half * v,
        const __half * mask,
...
#pragma unroll
        for (int r_local = 0; r_local < 4; ++r_local) {
            ...
            const float row_max_part = pyre_wave_reduce_max_wmma16(score);
            if (lane == 0) {
...
#pragma unroll
    for (int r_local = 0; r_local < 4; ++r_local) {
        const float l_part = pyre_wave_reduce_sum_wmma16(l_frag[r_local]);
        if (lane == 0) {
