...
struct pyre_block_q4_K_moe_mmq {
    unsigned short d;
    unsigned short dmin;
    uint8_t scales[12];
    uint8_t qs[128];
...
struct pyre_block_q8_1_x4_moe_mmq {
    unsigned short ds[8];
    int qs[32];
};
...
struct pyre_q4_k_moe_a_cache {
    int qs[8];
    float d;
    float min;
};

struct pyre_q8_1_moe_b_cache {
    int qs[8];
    float d;
    float s;
    uint32_t route;
...
struct pyre_q4_k_moe_a_pending {
    uint32_t qs_raw;
    unsigned short d;
    unsigned short dmin;
    uint8_t scale;
...
struct pyre_q8_1_moe_b_pending {
    int4 qs;
    unsigned short d;
    unsigned short s;
    uint32_t route;
...
static __device__ __forceinline__ void pyre_get_scale_min_k4_moe_mmq(
...
static __device__ __forceinline__ int pyre_udot4_q4_q8_1_moe_mmq(uint32_t qpack, int rpack) {
    return __builtin_amdgcn_sudot4(false, static_cast<int>(qpack), true, rpack, 0, false);
}

static __device__ __forceinline__ pyre_q4_k_moe_a_pending pyre_q4_k_moe_mmq_fetch_a(
        const pyre_block_q4_K_moe_mmq * src0,
        long long row,
        long long kb,
        int iqs,
        long long rows,
        ...
        long long src0_nb2,
        long long expert) {
    pyre_q4_k_moe_a_pending pending = {};
    ...
    const pyre_block_q4_K_moe_mmq * block = reinterpret_cast<const pyre_block_q4_K_moe_mmq *>(
        expert_base + row * src0_nb1 + block_idx * sizeof(pyre_block_q4_K_moe_mmq));
    ...
    if (iqs == 0) {
        ...
        pyre_get_scale_min_k4_moe_mmq(group, block->scales, &pending.scale, &pending.min);
    }
    return pending;
}

static __device__ __forceinline__ void pyre_q4_k_moe_mmq_commit_a(
        pyre_q4_k_moe_a_cache * buf_a,
        int buf_idx,
        const pyre_q4_k_moe_a_pending & pending,
        int iqs) {
    if (!pending.valid) {
...
static __device__ __forceinline__ pyre_q8_1_moe_b_pending pyre_q8_1_moe_mmq_fetch_b(
        const pyre_block_q8_1_x4_moe_mmq * src1,
    ...
    pyre_q8_1_moe_b_pending pending = {};
    ...
    const pyre_block_q8_1_x4_moe_mmq * rhs = src1 + (linear_block >> 2);
...
static __device__ __forceinline__ void pyre_q8_1_moe_mmq_commit_b(
        pyre_q8_1_moe_b_cache * buf_b,
        int buf_idx,
        const pyre_q8_1_moe_b_pending & pending,
        int iqs_vec4) {
...
extern "C" __global__ void pyre_mul_mat_id_q4_k_grouped_q8_1_x4_mmq64x64_wg64_f32(
        const pyre_block_q4_K_moe_mmq * src0,
        const pyre_block_q8_1_x4_moe_mmq * src1,
...
extern "C" __global__ void pyre_mul_mat_id_q4_k_swiglu_grouped_q8_1_x4_mmq32x64_wg64_f32(
        const pyre_block_q4_K_moe_mmq * gate,
        const pyre_block_q4_K_moe_mmq * up,
        const pyre_block_q8_1_x4_moe_mmq * src1,
        const uint32_t * counts,
        const uint32_t * routes,
        float * dst,
