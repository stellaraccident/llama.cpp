...
extern "C" __global__ void pyre_mul_mat_vec_bf16_swiglu_rows4_k2048_cols1_lds_wg256_f32(
        const uint16_t * gate,
        const uint16_t * up,
        const float * src1,
        float * dst,
...
#pragma unroll
        for (int iter = 0; iter < 16; ++iter) {
            ...
            gate_sum += pyre_bf16_swiglu_to_f32(static_cast<uint16_t>(g & 0xffffu)) * b.x;
            gate_sum += pyre_bf16_swiglu_to_f32(static_cast<uint16_t>(g >> 16)) * b.y;
            up_sum += pyre_bf16_swiglu_to_f32(static_cast<uint16_t>(u & 0xffffu)) * b.x;
            up_sum += pyre_bf16_swiglu_to_f32(static_cast<uint16_t>(u >> 16)) * b.y;
        }
    }
