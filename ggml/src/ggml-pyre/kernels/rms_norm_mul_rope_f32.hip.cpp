...
struct pyre_rms_norm_mul_rope_constants {
    long long ncols;
    long long nrows;
    long long ne1;
    long long ne2;
...
struct pyre_rms_norm_mul_rope_set_rows_constants {
    long long ncols;
    long long nrows;
    long long ne1;
    long long ne2;
...
static __device__ long long pyre_load_i64(const long long * base, long long byte_offset) {
    return *reinterpret_cast<const long long *>(reinterpret_cast<const char *>(base) + byte_offset);
}

template <typename C>
static __device__ void pyre_row_coords(long long row, const C & c, long long & i1, long long & i2, long long & i3) {
    i3 = row / (c.ne1 * c.ne2);
...
template <typename C>
static __device__ int pyre_rope_pos_idx(int i0, const C & c) {
...
template <typename C>
static __device__ void pyre_store_rope_pair_f32(
    ...
    const int pos_idx = pyre_rope_pos_idx(i0, c);
...
static __device__ void pyre_store_rope_pair_f16_set_rows(
        const float * normed, const int * pos, const long long * idxs, __half * dst,
        const pyre_rms_norm_mul_rope_set_rows_constants & c,
        long long i1, long long i2, long long i3, int i0) {
    const long long i12 = i3 % c.set_rows_ne12;
    const long long i11 = i2 % c.set_rows_ne11;
    const long long dst_row = pyre_load_i64(idxs, i2 * c.idx_nb0 + i11 * c.idx_nb1 + i12 * c.idx_nb2);
...
template <typename C>
static __device__ void pyre_compute_normed(
...
extern "C" __global__ void pyre_rms_norm_mul_rope_f32(
        const float * src, const float * weight, const int * pos, float * dst,
        pyre_rms_norm_mul_rope_constants c) {
    ...
    long long i2 = 0;
    long long i3 = 0;
    pyre_row_coords(row, c, i1, i2, i3);
    pyre_compute_normed(src, weight, normed, c, i1, i2, i3, tid);

    for (int i0 = static_cast<int>(2 * tid); i0 < c.ncols; i0 += 1024) {
        pyre_store_rope_pair_f32(normed, pos, dst, c, i1, i2, i3, i0);
    }
}

extern "C" __global__ void pyre_rms_norm_mul_rope_set_rows_f32_f16(
        const float * src, const float * weight, const int * pos, const long long * idxs, __half * dst,
        pyre_rms_norm_mul_rope_set_rows_constants c) {
    const long long row = __builtin_amdgcn_workgroup_id_x();
    const unsigned int tid = __builtin_amdgcn_workitem_id_x();
    if (row >= c.nrows) {
        return;
    ...
    for (int i0 = static_cast<int>(2 * tid); i0 < c.ncols; i0 += 1024) {
        pyre_store_rope_pair_f16_set_rows(normed, pos, idxs, dst, c, i1, i2, i3, i0);
    }
}
