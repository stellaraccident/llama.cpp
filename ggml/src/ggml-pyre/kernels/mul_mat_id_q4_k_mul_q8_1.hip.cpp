...
struct pyre_block_q8_1_id_mul {
    unsigned short d;
    unsigned short s;
    int8_t qs[32];
};

struct pyre_mul_mat_id_q4_k_mul_q8_1_constants {
    long long k;
    long long rows;
    long long n_ids;
    long long n_tokens;
...
extern "C" __global__ void pyre_mul_mat_id_q4_k_mul_q8_1_f32(
        const pyre_block_q4_K_id_mul * src0,
        const pyre_block_q8_1_id_mul * src1,
        const int * ids,
        const float * scale,
        float * dst,
        pyre_mul_mat_id_q4_k_mul_q8_1_constants c) {
    ...
    const pyre_block_q8_1_id_mul * src1_col = src1 + src1_col_idx * (c.k / 32);
    float sum = 0.0f;
    ...
    for (long long block_idx = block_slot; block_idx < blocks_per_row; block_idx += 4) {
        ...
        const pyre_block_q8_1_id_mul * rhs = src1_col + block_idx * 8 + group;
