...
struct pyre_sigmoid_mul_f32_strided_constants {
    long long ne0;
    long long nrows;
    long long attn_ne0;
    long long attn_ne1;
...
extern "C" __global__ void pyre_sigmoid_mul_f32_strided(
        const float * attn,
        const float * gate,
        float * dst,
        pyre_sigmoid_mul_f32_strided_constants c) {
    ...
    dst[idx] =
        *reinterpret_cast<const float *>(attn_ptr) * pyre_sigmoid_value_f32(g);
}
