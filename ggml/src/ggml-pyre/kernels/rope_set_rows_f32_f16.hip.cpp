...
struct pyre_rope_set_rows_f32_f16_constants {
    long long ne00;
    long long ne01;
    long long ne02;
    long long nrows;
...
static __device__ void pyre_store_f16(__half * base, long long byte_offset, float value) {
    *reinterpret_cast<__half *>(reinterpret_cast<char *>(base) + byte_offset) = __float2half(value);
}

extern "C" __global__ void pyre_rope_set_rows_f32_f16(
        const float * src, const int * pos, const long long * idxs, __half * dst,
        pyre_rope_set_rows_f32_f16_constants c) {
    ...
    if (i0 >= c.n_dims) {
        pyre_store_f16(dst, flat_base_bytes + i0 * static_cast<long long>(sizeof(__half)), src[src_base + i0 + 0]);
        pyre_store_f16(dst, flat_base_bytes + (i0 + 1) * static_cast<long long>(sizeof(__half)), src[src_base + i0 + 1]);
        return;
    }
    ...
    pyre_store_f16(dst, flat_base_bytes + off0 * static_cast<long long>(sizeof(__half)), x0 * cos_theta - x1 * sin_theta);
    pyre_store_f16(dst, flat_base_bytes + off1 * static_cast<long long>(sizeof(__half)), x0 * sin_theta + x1 * cos_theta);
}
