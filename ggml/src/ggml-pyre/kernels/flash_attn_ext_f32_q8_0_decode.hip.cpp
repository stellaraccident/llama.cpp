...
struct pyre_flash_attn_ext_f32_q8_0_decode_constants {
    long long D;
    long long KV;
    long long N;
    long long H;
...
static __device__ __forceinline__ float pyre_load_q8_0(const pyre_block_q8_0 * base, long long d) {
    const pyre_block_q8_0 * block = base + d / 32;
...
static __device__ __forceinline__ double pyre_alibi_slope(
        const pyre_flash_attn_ext_f32_q8_0_decode_constants c,
        long long head) {
    if (c.max_bias <= 0.0f) {
        return 1.0;
    }
...
extern "C" __global__ void pyre_flash_attn_ext_f32_q8_0_decode(
        const float * q, const pyre_block_q8_0 * k, const pyre_block_q8_0 * v,
        const __half * mask, const float * sinks, float * dst,
        pyre_flash_attn_ext_f32_q8_0_decode_constants c) {
    ...
    for (long long t = tid; t < c.KV; t += 256) {
        ...
        for (long long d = 0; d < c.D; ++d) {
            ...
            score += qv * pyre_load_q8_0(reinterpret_cast<const pyre_block_q8_0 *>(k_row), d);
        }
        score *= c.scale;
    ...
    for (long long d = 0; d < c.D; ++d) {
        ...
        for (long long t = tid; t < c.KV; t += 256) {
            ...
            local += logits[t] * inv_sum * pyre_load_q8_0(reinterpret_cast<const pyre_block_q8_0 *>(v_row), d);
        }
