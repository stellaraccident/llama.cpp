...
struct pyre_set_rows_constants {
    long long nc;
    long long nr;
    long long ne02;
    long long ne03;
...
static __device__ __forceinline__ uint8_t pyre_quantize_q4_0(float v) {
    const int q = static_cast<int>(v + 8.5f);
    return static_cast<uint8_t>(q < 0 ? 0 : (q > 15 ? 15 : q));
}

extern "C" __global__ void pyre_set_rows_f32_q4_0(
        const float * src0, const long long * idxs, pyre_block_q4_0 * dst,
        pyre_set_rows_constants c) {
    ...
    pyre_block_q4_0 * block = reinterpret_cast<pyre_block_q4_0 *>(
        reinterpret_cast<char *>(dst) + ib0 * static_cast<long long>(sizeof(pyre_block_q4_0)) +
    ...
    for (int j = 0; j < 16; ++j) {
        ...
        const uint8_t q0 = pyre_quantize_q4_0(v0 * id);
        const uint8_t q1 = pyre_quantize_q4_0(v1 * id);
        block->qs[j] = q0 | (q1 << 4);
    }
}
