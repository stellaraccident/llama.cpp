...
struct pyre_flash_attn_ext_f32_f16_prefill_tile8_constants {
    long long D;
    long long KV;
    long long N;
    long long H;
...
static __device__ __forceinline__ float pyre_load_f16_tile8(const __half * base, long long byte_offset) {
    return __half2float(*reinterpret_cast<const __half *>(reinterpret_cast<const char *>(base) + byte_offset));
}

static __device__ __forceinline__ float pyre_alibi_slope_tile8(
        const pyre_flash_attn_ext_f32_f16_prefill_tile8_constants c,
        long long head) {
    if (c.max_bias <= 0.0f) {
        return 1.0f;
    }
...
extern "C" __global__ void pyre_flash_attn_ext_f32_f16_prefill_tile8(
        const float * q,
        const __half * k,
        const __half * v,
        const __half * mask,
        const float * sinks,
        float * dst,
        pyre_flash_attn_ext_f32_f16_prefill_tile8_constants c) {
    constexpr int BR = 8;
    ...
    const float slope = pyre_alibi_slope_tile8(c, head);
...
#pragma unroll
    ...
    for (long long idx = tid; idx < BR * c.KV; idx += WG) {
        ...
        if (c.has_mask) {
            mask_value = pyre_load_f16_tile8(reinterpret_cast<const __half *>(mask_row), t * c.mask_nb0);
...
#pragma unroll
        for (long long d = 0; d < 256; ++d) {
            ...
            score += qv * pyre_load_f16_tile8(reinterpret_cast<const __half *>(k_row), d * static_cast<long long>(sizeof(__half)));
        }
        score *= c.scale;
...
#pragma unroll 4
            for (long long t = 0; t < 512; ++t) {
                ...
                local += logits[r][t] * inv_sum *
                    pyre_load_f16_tile8(reinterpret_cast<const __half *>(v_row), tid * static_cast<long long>(sizeof(__half)));
            }
