...
struct pyre_block_q4_K_q8_1_lhs {
    unsigned short d;
    unsigned short dmin;
    uint8_t scales[12];
    uint8_t qs[128];
...
struct pyre_block_q8_1_rhs {
    unsigned short d;
    unsigned short s;
    int8_t qs[32];
};

static __device__ __forceinline__ void pyre_get_scale_min_k4_q8_1(
...
static __device__ __forceinline__ float pyre_reduce_256_q8_1(float sum, float * shared) {
...
extern "C" __global__ void pyre_mul_mat_vec_q4_k_q8_1_f32(
        const pyre_block_q4_K_q8_1_lhs * src0,
        const pyre_block_q8_1_rhs * src1,
        float * dst,
    ...
    const pyre_block_q4_K_q8_1_lhs * row_blocks = src0 + row * blocks_per_row;
    const pyre_block_q8_1_rhs * src1_col = src1 + col * (k / 32);
    float sum = 0.0f;
    ...
    for (long long block_idx = block_slot; block_idx < blocks_per_row; block_idx += 4) {
        const pyre_block_q4_K_q8_1_lhs * block = row_blocks + block_idx;
        const pyre_block_q8_1_rhs * rhs = src1_col + block_idx * 8 + group;

        uint8_t sc = 0;
        uint8_t m = 0;
        pyre_get_scale_min_k4_q8_1(group, block->scales, &sc, &m);
    ...
    sum = pyre_reduce_256_q8_1(sum, sumsh);
