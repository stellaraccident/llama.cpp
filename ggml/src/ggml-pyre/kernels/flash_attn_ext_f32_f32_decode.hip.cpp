...
struct pyre_flash_attn_ext_f32_f32_decode_constants {
    long long D;
    long long KV;
    long long N;
    long long H;
...
static __device__ __forceinline__ float pyre_load_f32(const float * base, long long byte_offset) {
...
static __device__ __forceinline__ double pyre_alibi_slope(
        const pyre_flash_attn_ext_f32_f32_decode_constants c,
        long long head) {
    if (c.max_bias <= 0.0f) {
        return 1.0;
    }
...
extern "C" __global__ void pyre_flash_attn_ext_f32_f32_decode(
        const float * q, const float * k, const float * v, const __half * mask, const float * sinks, float * dst,
        pyre_flash_attn_ext_f32_f32_decode_constants c) {
    ...
    for (long long t = tid; t < c.KV; t += 256) {
        ...
        for (long long d = 0; d < c.D; ++d) {
            ...
            score += qv * pyre_load_f32(reinterpret_cast<const float *>(k_row), d * static_cast<long long>(sizeof(float)));
        }
        score *= c.scale;
    ...
    for (long long d = 0; d < c.D; ++d) {
        ...
        for (long long t = tid; t < c.KV; t += 256) {
            ...
            local += logits[t] * inv_sum *
                pyre_load_f32(reinterpret_cast<const float *>(v_row), d * static_cast<long long>(sizeof(float)));
        }
