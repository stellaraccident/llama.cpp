...
def main():
    ...
    if not args.hsaco_dir:
        write_catalog(output, [], "HRX pure-HIP kernel catalog disabled: no hsaco directory configured.")
        return 0
    if not args.arch:
        write_catalog(output, [], "HRX pure-HIP kernel catalog disabled: no AMDGPU architecture configured.")
        return 0
    ...
    for kernel in KERNELS:
        ...
        if source not in hsaco_cache:
            ...
            if not hsaco.exists():
                ...
                return 1
            hsaco_cache[source] = {
                "symbol": f"k_hrx_kernel_{len(hsaco_cache)}",
