...
template <int COLS>
static __device__ __forceinline__ void hrx_q6_k_dot4_cols_acc(
        const hrx_block_q6_K * block,
        const float * src0,
        long long k,
        float d,
...
template <int COLS>
static __device__ __forceinline__ void hrx_q6_k_dot4_cols_acc_full(
        const hrx_block_q6_K * block,
        const float * src0,
        long long k,
        float d,
...
template <int COLS>
static __device__ __forceinline__ void hrx_mul_mat_vec_q6_k_rows2_cols_wg32_f32_impl(
        const hrx_block_q6_K * src0, const float * src1, float * dst,
        long long k, long long rows, long long cols) {
    const long long row0 = static_cast<long long>(__builtin_amdgcn_workgroup_id_x()) * 2;
    ...
    for (long long block_idx = block_slot; block_idx < blocks_per_row; block_idx += 2) {
        if (full_cols) {
            #pragma unroll
            for (int col = 0; col < COLS; ++col) {
                const float * src_block = src1 + (col0 + col) * k + block_idx * 256;
                sum[col] += hrx_q6_k_dot16(row0_blocks + block_idx, src_block, itid);
                sum[COLS + col] += hrx_q6_k_dot16(row1_blocks + block_idx, src_block, itid);
            }
        } else {
            #pragma unroll
            for (int col = 0; col < COLS; ++col) {
...
static __device__ __forceinline__ void hrx_mul_mat_vec_q6_k_rows2_cols_f32_impl(
        const hrx_block_q6_K * src0, const float * src1, float * dst,
        long long k, long long rows, long long cols) {
    const long long row0 = static_cast<long long>(__builtin_amdgcn_workgroup_id_x()) * 2;
    const long long col0 = static_cast<long long>(__builtin_amdgcn_workgroup_id_y()) * COLS;
    ...
    for (long long block_idx = block_slot; block_idx < blocks_per_row; block_idx += block_stride) {
        ...
        if (full_cols) {
            hrx_q6_k_dot4_cols_acc_full<COLS>(block0, src1_col0 + src_base, k, d0, group, lane, sum, 0);
            hrx_q6_k_dot4_cols_acc_full<COLS>(block1, src1_col0 + src_base, k, d1, group, lane, sum, COLS);
        } else {
            hrx_q6_k_dot4_cols_acc<COLS>(block0, src1_col0 + src_base, k, d0, group, lane, sum, 0, valid_cols);
            hrx_q6_k_dot4_cols_acc<COLS>(block1, src1_col0 + src_base, k, d1, group, lane, sum, COLS, valid_cols);
        }
    }
...
#define HRX_Q6_ROWS2_COLS_ENTRY(COLS) \
extern "C" __global__ void hrx_mul_mat_vec_q6_k_rows2_cols##COLS##_wg128_f32( \
        const hrx_block_q6_K * src0, const float * src1, float * dst, \
        long long k, long long rows, long long cols) { \
    hrx_mul_mat_vec_q6_k_rows2_cols_f32_impl<COLS, 128>(src0, src1, dst, k, rows, cols); \
} \
extern "C" __global__ void hrx_mul_mat_vec_q6_k_rows2_cols##COLS##_wg64_f32( \
        const hrx_block_q6_K * src0, const float * src1, float * dst, \
        long long k, long long rows, long long cols) { \
    hrx_mul_mat_vec_q6_k_rows2_cols_f32_impl<COLS, 64>(src0, src1, dst, k, rows, cols); \
}

HRX_Q6_ROWS2_COLS_ENTRY(2)
HRX_Q6_ROWS2_COLS_ENTRY(3)
HRX_Q6_ROWS2_COLS_ENTRY(4)
HRX_Q6_ROWS2_COLS_ENTRY(5)
HRX_Q6_ROWS2_COLS_ENTRY(6)
HRX_Q6_ROWS2_COLS_ENTRY(7)
HRX_Q6_ROWS2_COLS_ENTRY(8)

#undef HRX_Q6_ROWS2_COLS_ENTRY

#define HRX_Q6_ROWS2_COLS_WG32_ENTRY(COLS) \
extern "C" __global__ void hrx_mul_mat_vec_q6_k_rows2_cols##COLS##_wg32_f32( \
        const hrx_block_q6_K * src0, const float * src1, float * dst, \
        long long k, long long rows, long long cols) { \
    hrx_mul_mat_vec_q6_k_rows2_cols_wg32_f32_impl<COLS>(src0, src1, dst, k, rows, cols); \
}

HRX_Q6_ROWS2_COLS_WG32_ENTRY(2)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(3)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(4)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(5)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(6)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(7)
HRX_Q6_ROWS2_COLS_WG32_ENTRY(8)

#undef HRX_Q6_ROWS2_COLS_WG32_ENTRY
