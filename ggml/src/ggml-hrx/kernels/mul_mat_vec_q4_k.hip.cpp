...
extern "C" __global__ void hrx_mul_mat_vec_q4_k_wg128_f32(
        const hrx_block_q4_K * src0, const float * src1, float * dst,
        long long k, long long rows, long long cols) {
    hrx_mul_mat_vec_q4_k_f32_impl<128>(src0, src1, dst, k, rows, cols);
}

extern "C" __global__ void hrx_mul_mat_vec_q4_k_wg64_f32(
        const hrx_block_q4_K * src0, const float * src1, float * dst,
        long long k, long long rows, long long cols) {
    hrx_mul_mat_vec_q4_k_f32_impl<64>(src0, src1, dst, k, rows, cols);
}

extern "C" __global__ void hrx_mul_mat_vec_q4_k_packed_wg64_f32(
        const hrx_block_q4_K * src0, const float * src1, float * dst,
        long long k, long long rows, long long cols) {
    const long long row = __builtin_amdgcn_workgroup_id_x();
    const long long col = __builtin_amdgcn_workgroup_id_y();
    ...
    for (long long block_idx = block_slot; block_idx < blocks_per_row; block_idx += 4) {
        ...
        uint8_t sc3 = 0;
        uint8_t m3 = 0;
        hrx_get_scale_min_k4(g0, block->scales, &sc0, &m0);
        hrx_get_scale_min_k4(g1, block->scales, &sc1, &m1);
        hrx_get_scale_min_k4(g2, block->scales, &sc2, &m2);
        hrx_get_scale_min_k4(g3, block->scales, &sc3, &m3);
